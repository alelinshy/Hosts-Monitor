...
- **规则名** 对应 hosts 文件中的 `# 注释名`（如 `# Block Ads`）。
- **IP + 域名** 对应 hosts 文件中的具体条目（如 `127.0.0.1 example.com`）。

2. 规则写入逻辑

- 如果 hosts 文件中无匹配项（既无相同注释名，也无相同 IP + 域名组合），则在文件 末尾写入规则，并确保规则上下各保留一行空行。
- 如果 hosts 文件中有匹配项（注释名或 IP + 域名相同）：
  - 以第一个匹配到的位置作为基准点，按规则顺序覆盖更新后续内容。
  - 删除后续所有重复的注释名和IP + 域名条目（避免重复生效）。

3. 格式规范

- **每条规则前后必须各有一行空行**（如有多余空行则删除，仅保留单行间隔）。

4. 特殊情况处理

- **注释名匹配但 IP + 域名不匹配**：以注释名为基准点，覆盖其后的 IP + 域名部分。
- **IP + 域名匹配但无注释名**：在匹配的 IP + 域名前插入规则注释名，并修正格式。
...
def auto_log(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        logger.info(f'函数 {func.__name__} 返回值: {result}')
        return result
    return wrapper

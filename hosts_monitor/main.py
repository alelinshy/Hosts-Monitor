...
def run_as_task() -> bool:
    ...
    try:
        import traceback
        from .utils import check_task_exists, run_task, create_scheduled_task, delete_scheduled_task
        ...
        if "--skip-admin-check" not in current_args:
            current_args += " --skip-admin-check" if current_args else "--skip-admin-check"
...
def check_and_run_as_admin() -> bool:
    ...
    try:
        ...
        # 记录启动信息，用于调试
        logger.info(f"启动信息 - 管理员权限: {admin_status}, 重启标识: {is_restarting}, UAC提权: {is_uac_restart}")
        ...
        if config.get("general", "run_as_admin", False) and not admin_status:
            ...
            try:
                ...
                if paths['is_frozen']:
                    ...
                    # 尝试找到项目根目录下的run.py
                    run_script_path = os.path.abspath(os.path.join(paths['app_dir'], '..', 'run.py'))
                ...
                if run_as_admin(app_path=app_path, app_args=app_args, work_dir=work_dir):
...
def create_restart_task(update=False, delete_after_run=False) -> bool:
    ...
    try:
        # 导入工具函数
        from .utils import check_task_exists, create_scheduled_task, delete_scheduled_task
